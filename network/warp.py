...
def meshgrid(img, device, homogeneous=False):
    ...
    x_range = torch.arange(0, w, device=device).view(1, 1, w).expand(1, h, w).type_as(img)  # [1, H, W]
    y_range = torch.arange(0, h, device=device).view(1, h, 1).expand(1, h, w).type_as(img)
...
def disp_warp(img, disp, device, padding_mode='border'):
    ...
    grid = meshgrid(img, device)  # [B, 2, H, W] in image scale
    # Note that -disp here
