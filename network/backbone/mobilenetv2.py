...
class MobileNetV2(nn.Module):
    def __init__(self, num_classes=1000, output_stride=8, width_mult=1.0, inverted_residual_setting=None, round_nearest=8):
        ...
        for idx, (t, c, n, s) in enumerate(inverted_residual_setting):
            ...
            layer = block if idx < 3 else block2    # adapt deformable convolution to high-level features

            for i in range(n):
                if i==0:
                    features.append(layer(input_channel, output_channel, stride, previous_dilation, expand_ratio=t))
                else:
                    features.append(layer(input_channel, output_channel, 1, dilation, expand_ratio=t))
