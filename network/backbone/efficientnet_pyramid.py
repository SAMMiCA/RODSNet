...
from network.efficientnet_pytorch.utils import (
    round_filters,
    round_repeats,
    drop_connect,
...
class EfficientNet(nn.Module):
    ...
    def __init__(self, blocks_args=None, global_params=None, **kwargs):
        ...
        # self.img_mean = torch.tensor(mean).view(1, -1, 1, 1)
        # self.img_std = torch.tensor(std).view(1, -1, 1, 1)
        ...
        bottlenecks = []
        bottlenecks += [convkxk(24, self.num_features, k=1)]
        bottlenecks += [convkxk(40, self.num_features, k=1)]
        bottlenecks += [convkxk(112, self.num_features, k=1)]
        bottlenecks += [convkxk(320, self.num_features, k=1)]
        ...
        self.fine_tune = [self._conv_stem, self._bn0, self._blocks, self._conv_head, self._bn1, self._swish]
    ...
    def extract_features(self, inputs, skips, skip_idx=-1):
        ...
        for idx, block in enumerate(self._blocks):
            ...
            if drop_connect_rate:
                drop_connect_rate *= float(idx) / len(self._blocks)  # scale drop connect_rate
            x = block(x, drop_connect_rate=drop_connect_rate)
            if idx in dimension_reduced:
                features += [x]     # (/4, 24), (/8, 40), (/16, 112), (/32, 320)
    ...
    def forward(self, inputs):
        ...
        inputs -= self.img_mean
        inputs /= self.img_std
        pyramid = [inputs]
        for l in range(1, self.pyramid_levels):
            pyramid += [F.interpolate(inputs, scale_factor=1 / 2 ** l, mode=self.pyramid_subsample,
                                      align_corners=self.align_corners)]

        skips = [[] for _ in range(self.num_skip_levels)]
        additional = {'pyramid': pyramid}
        for idx, p in enumerate(pyramid):
            skips = self.extract_features(p, skips, skip_idx=idx)
        skips = skips[::-1]
        x = skips[0][0]
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        super(EfficientNet, self)._load_from_state_dict(state_dict, prefix, local_metadata, strict, missing_keys,
...
def efficientnet_pyramid(pretrained=True, num_classes=1000, **kwargs):
    """Constructs a ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """

    # model = EfficientNet(BasicBlock, [2, 2, 2, 2], **kwargs)

    # choose efficientnet-b0 ~ b7
    efficient_model = 'efficientnet-b0'

    if pretrained:
        model = EfficientNet.from_pretrained(efficient_model, num_classes=num_classes)
        print("=> using pre-trained model '{}'".format(efficient_model))
    else:
        print("=> creating model '{}'".format(efficient_model))
        model = EfficientNet.from_name(efficient_model)
